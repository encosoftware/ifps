...
namespace operations_research {
	...
	std::ostream& operator<< (std::ostream& out, const EdgingPlan& plan) {
	...
	int EdgingPlan::EstimateTime(EdgingWorkstation ws) {

		float time_estimate = std::ceil(ws.GetSecPerEdge() * FoilNum / 60);

		return std::max(int(time_estimate), 1);
	}

	std::string EdgingPlan::PrintHtml(sat::CpSolverResponse result, int minute_pixel_rate) {
	...
	json EdgingPlan::ToJson(sat::CpSolverResponse result, int wsh, int shift_num, int shift_len) {
