...
namespace operations_research {
	...
	std::ostream& operator<< (std::ostream& out, const SortingPlan& plan) {
	...
	int SortingPlan::EstimateTime(SortingWorkstation ws) {

		int component_num = ComponentIds.size();
		float time_estimate = std::ceil(component_num * ws.GetSortingSecPerComponent() / 60);

		return std::max(int(time_estimate), 1);
	}

	std::string SortingPlan::PrintHtml(sat::CpSolverResponse result, int minute_pixel_rate) {
	...
	json SortingPlan::ToJson(sat::CpSolverResponse result, int wsh, int shift_num, int shift_len) {
