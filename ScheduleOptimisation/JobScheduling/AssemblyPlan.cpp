...
namespace operations_research {
	...
	std::ostream& operator<< (std::ostream& out, const AssemblyPlan& plan) {
		...
		out << std::endl;
		out << "     Accessories:    " << plan.Accessories << " pcs" << std::endl;
		out << "     Execution time: " << plan.Schedule.SizeVar() << " min";

		return out;
	}

	int AssemblyPlan::EstimateTime(AssemblyWorkstation ws) {

		int component_num = ComponentIds.size();
		float time_estimate = std::ceil((component_num * ws.GetAssemblySecPerComponent() + Accessories * ws.GetSecPerAccessory()) / 60);

		return std::max(int(time_estimate), 1);
	}

	std::string AssemblyPlan::PrintHtml(sat::CpSolverResponse result, int minute_pixel_rate) {
	...
	json AssemblyPlan::ToJson(sat::CpSolverResponse result, int wsh, int shift_num, int shift_len) {
