...
COPY frontend/web/ .
COPY --from=tsGen-stage /app/generatedClient/index.ts ./projects/sales/src/app/shared/clients/

RUN gulp iconfont --cwd ./projects/sales/src
#RUN npm rebuild node-sass
RUN yarn build butor-shared-lib
RUN yarn build sales -c prodlike

# production stage
FROM nginx:1.16.1-alpine as production-stage
COPY --from=build-stage /app/dist/sales /usr/share/nginx/html
