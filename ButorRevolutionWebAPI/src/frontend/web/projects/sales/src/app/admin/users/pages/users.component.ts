...
export class UsersComponent implements OnInit, OnDestroy, AfterViewInit {
  ...
  dataSource: PagedData<IUsersListViewModel> = {
    items: [],
    pageIndex: 0,
    pageSize: 15
  };
  ...
  loadImage(containerName: string | undefined, fileName: string | undefined) {
    if (containerName && fileName) {
      this.PicService.getThumbnail(containerName, fileName)
        .pipe(
          tap(resp => {
  ...
  deleteUser(data: IUsersListViewModel) {
    this.userListService
      .deleteUsers(data.id).pipe(
        catchError((err:HttpErrorResponse) => {
          if (err.status === 400) {
            this.snackbarService.customMessage(this.translate.instant('snackbar.userCantDelete'));
          }
          return of();
        })
      )
  ...
  activateOrDisable(data: IUsersListViewModel) {
    return data.status
      ? this.translate.instant('Users.EditUser.buttons.Deactivate')
      : this.translate.instant('Users.EditUser.buttons.Activate');
  }
}
