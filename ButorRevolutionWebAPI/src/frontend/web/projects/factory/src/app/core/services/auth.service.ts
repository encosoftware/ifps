import { JwtTokenModel, TokenModel, LoginViewModel } from '../models/auth';
...
import { Claims, Token, LoginToken } from '../store/actions/core.actions';
...
export class AuthService {
  ...
  changePassword(userId: number, password: string): Observable<void> {
    let pUpdateDto = new UserPasswordUpdateDto({
      currentPassword: password,
      newPassword: password      
    });

    return this.passwordChange.changePassword(userId, pUpdateDto);
  }
