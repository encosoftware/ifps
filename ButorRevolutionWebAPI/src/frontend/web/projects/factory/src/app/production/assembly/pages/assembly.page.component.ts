...
import { IAssemblyListViewModel } from '../models/assembly.model';
import { Store, select } from '@ngrx/store';
import { AssemblyService } from '../services/assembly.service';
import { tap, debounceTime, takeUntil, switchMap, map, catchError, finalize } from 'rxjs/operators';
import { ChangeFilter, DeleteFilter } from '../store/actions/assembly.actions';
import { assemblyFilters } from '../store/selectors/assembly-list.selector';
import { ProductionService } from '../../../shared/services/production.service';

@Component({
    templateUrl: './assembly.page.component.html',
    styleUrls: ['./assembly.page.component.scss']
})
export class AssemblyPageComponent implements OnDestroy, AfterViewInit, OnInit {
    destroy$ = new Subject();
    ...
    dataSource: PagedData<IAssemblyListViewModel> = { items: [], pageIndex: 0, pageSize: 15 };
    ...
    loadData() {
        ...
        this.assemblyForm.valueChanges.pipe(
            debounceTime(500),
        ...
        this.store.pipe(
            select(assemblyFilters),
            tap(val => this.assemblyForm.patchValue(val, { emitEvent: false })),
            tap(() => this.isLoading = true),
            switchMap((filter) =>
                this.assemblyService.getAssemblies(filter).pipe(
