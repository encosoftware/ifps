...
namespace Cotur.DataMining.AssociationMining
{
    ...
    public class Apriori
    {
        ...
        public int MaxCombinationLength;
        public Apriori(DataFields Data, int maxComboLength)
        {
            this.Data = Data;
            this.MaxCombinationLength = maxComboLength;
        }
        public void CalculateCNodes(float minSupport, string keyDelimiter)
        {
            this.CNodes = null;
            this.EachLevelOfNodes = new List<List<CNode>>();
            this.Rules = null;
            _CalculateCNodes(minSupport, keyDelimiter);
        }
        private void _CalculateCNodes(float minSupport, string keyDelimiter, List<CNode> cNodes = null)
        {
            ...
            if (this.CNodes.Count > 1 && CalculateNextStep(minSupport) && EachLevelOfNodes.Count <= this.MaxCombinationLength)
            {
                _CalculateCNodes(minSupport, keyDelimiter, this.CNodes);
            }
            else
            {
                Rules = AssociationRule.GetAllNormalRules(EachLevelOfNodes, this.MaxCombinationLength, keyDelimiter);
            }            
        }
    ...
    public class AssociationRule
    {
        ...
        // rule for: A -> B
        public AssociationRule(CNode NodeAB, CNode NodeA, CNode NodeB, string keyDelimiter)
        {
            this.NodeAB = NodeAB;
            this.NodeA = NodeA;
            this.NodeB = NodeB;
        ...
        {
            float lift = Confidence / NodeB.Support;
            if(float.IsNaN(lift))
            {
                lift = 0f;
            }
        ...
        public string GetDictKey(DataFields data)
        {
            return string.Join(KeyDelimiter, data.GetElementsName(NodeA.ElementIDs));
        }
        ...
        public static List<AssociationRule> GetAllNormalRules(List<List<CNode>> EachLevelOfCNodes, int maxCombinationLength, string keyDelimiter)
        {
            List<AssociationRule> Rules = new List<AssociationRule>();

            // only calculate for the combinations of the given length
            for (int i = 1; i <= maxCombinationLength; i++)
            {
                foreach (var Node in EachLevelOfCNodes[i]) // node level
                {
                    Rules.AddRange(GetNormalRules(Node, EachLevelOfCNodes, keyDelimiter));
                }
            }
        ...
        public static List<AssociationRule> GetNormalRules(CNode NodeAB, List<List<CNode>> EachLevelOfCNodes, string keyDelimiter)
        {
            List<AssociationRule> Rules = new List<AssociationRule>();
            List<IEnumerable<int>> subsets = SubSetsOf<int>(NodeAB.ElementIDs).ToList();
            int l = NodeAB.ElementIDs.Count - 1; // length of relevant subsets at level
            var relevant_nodes = subsets.Where(x => x.Count() == l).ToList();
            ...
            foreach (var keyIds in relevant_nodes)
            {
                nodeA = new CNode(keyIds.ToList());
                var valueId = NodeAB.ElementIDs.Except(keyIds.ToList()).First();
                nodeB = new CNode(valueId);
                ...
                {
                    foreach (CNode node in EachLevelOfCNodes[l - 1])
                    {
                    ...
                    foreach (CNode node in EachLevelOfCNodes[0])
                    {
                    ...
                    if (aFound && bFound)
                    {
                        Rules.Add(new AssociationRule(NodeAB, nodeA, nodeB, keyDelimiter).Calculate());
                    }
                }
            }
