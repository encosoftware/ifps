...
{
    public class PackingsAppService : ApplicationService, IPackingsAppService
    {
        private readonly IProductionProcessRepository productionProcessRepository;

        public PackingsAppService(IApplicationServiceDependencyAggregate aggregate,
        ...
        public async Task<PagedListDto<PackingListDto>> GetPackingsAsync(PackingFilterDto filterDto)
        {
            ...
            var orderingQuery = filterDto.Orderings.ToOrderingExpression<ProductionProcess>(PackingFilterDto.GetOrderingMapping(), nameof(ProductionProcess.Id));

            var allProductionProcess = await productionProcessRepository.GetPackingManualProductionProcessesAsync(filter, orderingQuery, filterDto.PageIndex, filterDto.PageSize);
            return allProductionProcess.ToPagedList(PackingListDto.FromEntity);
        }
    }
}
