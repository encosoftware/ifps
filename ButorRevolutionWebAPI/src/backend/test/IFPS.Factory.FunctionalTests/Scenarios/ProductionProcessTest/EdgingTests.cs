...
{
    public class EdgingTests : IClassFixture<IFPSFactoryWebApplicationFactory>
    {
        private readonly IFPSFactoryWebApplicationFactory factory;
        private readonly JsonSerializerSettings jsonSerializerSettings;

        public EdgingTests(IFPSFactoryWebApplicationFactory factory)
        {
        ...
        [Fact]
        public async Task Get_edging_by_production_process_should_work()
        {
            // Arrange
            var client = factory.CreateClient();
            var expectedResult = new List<EdgebandingListDto>();

            var firstEdge = new EdgebandingListDto(BuildProductionProcesses().First());
            expectedResult.Add(firstEdge);

            var secondEdge = new EdgebandingListDto(BuildProductionProcesses().Last());
            expectedResult.Add(secondEdge);

            IPagedList<EdgebandingListDto> pagedList = new PagedList<EdgebandingListDto>()
            {
            ...
            {
                var service = scope.ServiceProvider.GetRequiredService<IEdgebandingAppService>();
                var edgingFilterDto = new EdgebandingFilterDto() { OrderName = "ProductionProcess Test Order" };
                var edgings = await service.EdgebandingListAsync(edgingFilterDto);

                // Assert
                result.Should().BeEquivalentTo(edgings);
            }
        }

        [Fact]
        public async Task Get_edging_by_production_process_filter_component_name_should_not_work()
        {
            // Arrange
            var client = factory.CreateClient();
            ...
            {
                var service = scope.ServiceProvider.GetRequiredService<IEdgebandingAppService>();
                var edgingFilterDto = new EdgebandingFilterDto() { ComponentName = "Wrong Component Name" };
                var edgings = await service.EdgebandingListAsync(edgingFilterDto);

                // Assert
                edgings.Data.Count().Should().Be(0);
            }
        }

        [Fact]
        public async Task Get_edging_by_production_process_filter_order_name_should_not_work()
        {
            // Arrange
            var client = factory.CreateClient();
            ...
            {
                var service = scope.ServiceProvider.GetRequiredService<IEdgebandingAppService>();
                var edgingFilterDto = new EdgebandingFilterDto() { OrderName = "Wrooong!" };
                var edgings = await service.EdgebandingListAsync(edgingFilterDto);
